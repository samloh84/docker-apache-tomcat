...
def filter_latest_versions(versions,
    ...
    for version_element in versions:
        ...
        latest_version_element = _get(latest_versions, path)

        if latest_version_element is None:
            _set(latest_versions, path, version_element)
        else:
            latest_version_info = latest_version_element[2]
            if latest_version_info < version_info:
                _set(latest_versions, path, version_element)

    latest_versions = _flat_map_depth(latest_versions, depth=len(sort_criteria) - 1)
...
def get_latest_version(versions, version_constraints=None,
    ...
    latest_version = max(latest_versions, key=lambda latest_version: latest_version[2])
