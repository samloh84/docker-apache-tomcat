...
def update_apache_tomcat_data(data, config, update_all_versions=False):
...
def render_apache_tomcat_dockerfiles(data, config, update_all_versions=False, force_update=False):
...
def main(argv):
    ...
    parser.add_argument('--data-file', nargs='?', dest='data_file', default=os.path.abspath('apache_tomcat.yml'))
    ...
    if perform_update:
        data = update_apache_tomcat_data(data, config, update_all)
        write_yaml(data_file, data)

    render_apache_tomcat_dockerfiles(data, config, update_all, force_update)
