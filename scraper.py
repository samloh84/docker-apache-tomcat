...
class ApacheTomcatScraper:
    ...
    def list_version_files(self, version, return_unmatched_urls=False):
        ...
        for version in versions:
            normalized_version = normalize_version_to_semver(version)
            version_info = semver.parse_version_info(normalized_version)
            major_version = str(version_info.major)
            version_url = urlparse.urljoin(self.starting_url, 'tomcat-' + major_version + '/' + 'v' + version + '/')
            urls_to_parse.append(version_url)
            urls_to_parse.append(urlparse.urljoin(version_url, 'bin/'))
            urls_to_parse.append(urlparse.urljoin(version_url, 'src/'))
